...
public class DefaultActivityVisitor : IActivityVisitor
{
    ...
    /// <inheritdoc />
    public async Task<ActivityNode> VisitAsync(
        JsonObject activity,
    ...
    private async Task VisitRecursiveAsync(
        (ActivityNode Node, JsonObject Activity) pair,
        HashSet<JsonObject> collectedActivities,
        HashSet<ActivityNode> collectedNodes,
        CancellationToken cancellationToken
    )
    {
        await VisitPortsRecursiveAsync(
            pair,
            collectedActivities,
            collectedNodes,
            cancellationToken
    ...
    private async Task VisitPortsRecursiveAsync(
        (ActivityNode Node, JsonObject Activity) pair,
        HashSet<JsonObject> collectedActivities,
        HashSet<ActivityNode> collectedNodes,
        CancellationToken cancellationToken
    )
    {
        ...
        var embeddedActivities = await resolver.GetActivitiesAsync(
            pair.Activity,
            cancellationToken
        );
        ...
        foreach (var embeddedActivity in embeddedActivities)
        {
            ...
            var childNode = collectedNodes.FirstOrDefault(x =>
                x.Activity == embeddedActivity.Activity
            );

            if (childNode == null)
            {
                childNode = new ActivityNode(
                    embeddedActivity.Activity,
                    embeddedActivity.PropertyName
                );
            ...
            await VisitRecursiveAsync(
                (childNode, embeddedActivity.Activity),
                collectedActivities,
                collectedNodes,
                cancellationToken
            );
